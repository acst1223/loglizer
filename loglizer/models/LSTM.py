...
class LSTM(object):
    def __init__(self, g, h, L, alpha, batch_size, sym_count, lr=0.001, fc1_size=64):
        ...
        with tf.variable_scope('LSTM'):
            self.W = tf.get_variable('w', (self.alpha, self.sym_count), dtype=tf.float64, initializer=init)
        ...
        self.inputs = tf.placeholder(dtype=tf.float64, shape=(self.batch_size, self.h, self.sym_count))
        self.labels = tf.placeholder(dtype=tf.float64, shape=(self.batch_size, self.sym_count))
        output = self.forward(self.inputs)
        self.loss = tf.losses.softmax_cross_entropy(self.labels, output)
    ...
    def forward(self, inputs):
        ...
        _, final_hidden_state = tf.nn.static_rnn(multi_cell, inputs, initial_state=initial_state, dtype=tf.float64, scope='LSTM')
